...
import chat.paramvr.ws.Parameter
...
class ParameterDAO : DAO() {
    ...
    fun retrieveParameters(userId: Long, avatarVrcUuid: String?): List<Parameter> {
        ...
        connect().use { c ->
            c.prepareStatement(
                "select parameter.id, type, description, parameter.name, parameter.requires_invite, data_type," +
                        " default_value, min_value, max_value, press_value, saved, lockable, client_id" +
                ...
                while (rs.next()) {
                    ...
                    val requiresInvite = rs.getString(5) == "Y"
                    ...
                    val lockedByClientId = rs.getString(13)

                    val param = Parameter(desc, name, requiresInvite, type, dataType,
                        defaultValue, minValue, maxValue, pressValue, saved, lockable, lockedByClientId, parameterId)
                    parameters += param
                }
            }
        }
    ...
    fun retrieveParameters(userId: Long): List<GetParameter> {
        ...
        connect().use { c ->
            c.prepareStatement(
                "select parameter.id, vrc_uuid, avatar.name, type, description, parameter.name, requires_invite, data_type," +
                ...
                while (rs.next()) {
                    ...
                    val requiresInvite = rs.getString(7)
                    ...
                    val param = GetParameter(desc, name, requiresInvite, type, dataType,
    ...
    private fun retrieveParameterValues(id: Long): List<ParameterValue> {
        ...
        connect().use { c ->
            c.prepareStatement("select description, value, requires_invite" +
                    " from parameter_value where parameter_id = ?").use {
                it.setLong(1, id)
                val rs = it.executeQuery()
                while (rs.next()) {
                    values += ParameterValue(rs.getString(1),
                        rs.getString(2),
                        rs.getString(3) == "Y")
                }
            }
    ...
    fun insertParameter(userId: Long, param: PostParameter) {
        connect().use { c ->
            c.prepareStatement(
                "insert into parameter(user_id, avatar_id, type, description, name, requires_invite, data_type," +
                ...
                it.setString(6, param.requiresInvite())
    ...
    fun updateParameter(userId: Long, param: PostParameter) {
        connect().use { c ->
            c.prepareStatement(
                "update parameter set type = ?, description = ?, requires_invite = ?, data_type = ?, default_value = ?," +
                ...
                it.setString(3, param.requiresInvite())
    ...
    fun insertUpdateParameterValue(userId: Long, value: PostParameterValue) {
        ...
        connect().use { c ->
            val updated: Boolean
            c.prepareStatement("update parameter_value set description = ?, requires_invite = ? where parameter_id = ? and value = ?").use {
                it.setString(1, value.description)
                it.setString( 2, value.requiresInvite())
                it.setLong(3, value.parameterId)
                it.setString(4, value.value)
                updated = it.executeUpdate() > 0
            }
            if (!updated) {
                c.prepareStatement("insert into parameter_value(parameter_id, description, value, requires_invite) values (?, ?, ?, ?)").use {
                    it.setLong(1, value.parameterId)
                    it.setString(2, value.description)
                    it.setString(3, value.value)
                    it.setString(4, value.requiresInvite())
                    it.executeUpdate()
                }
            }
        }
    ...
    fun setParameterLock(listenerUserId: Long, parameterId: Long, locked: Boolean, lockedByClientId: String, inviteId: Long?): Boolean {
        ...
        connect().use { c ->
            if (locked) {
                c.prepareStatement(
                    "insert into locked_parameter(parameter_id, client_id, invite_id)" +
                        " select ?, ?, ? from dual" +
                        " where not exists (select 1 from locked_parameter where parameter_id = ?)"
                ).use {
                    it.setLong(1, parameterId)
                    ...
                    it.setLong(4, parameterId)
                    return it.executeUpdate() > 0
                }
            } else {
                c.prepareStatement("delete from locked_parameter where parameter_id = ? and client_id = ?").use {
                    it.setLong(1, parameterId)
                    it.setString(2, lockedByClientId)
                    return it.executeUpdate() > 0
                }
